from lsdb.models import MeasurementResult, MeasurementResult_pichina, Unit_pichina
...
def unit_history(unit):
    ...
    # This one lazy query
    count = MeasurementResult_pichina.objects.filter(step_result__procedure_result__unit=unit)
...
def unit_revenue(unit):
    ...
    queryset = unit.procedureresult_pichina_set.exclude(supersede=True,
...
def unit_minutes(unit):
    ...
    revenue = queryset.all().aggregate(revenue=Sum('procedure_definition__project_weight')).get('revenue')
    # Chuck empty modules:
    if queryset.filter(unit__procedureresult_pichina__disposition__isnull=False).count() == 0:
        return (0, 0)
    # determine "done to"
    done_to = queryset.all().annotate(done_to= Max('unit__procedureresult_pichina__linear_execution_group',
                filter=Q(unit__procedureresult_pichina__disposition__isnull=False)))\
                .distinct().first()
...
def measurements_completed(project):
    count = MeasurementResult_pichina.objects.filter(step_result__procedure_result__work_order__project=project)
    count = count.filter(disposition__isnull=False).count()
    return count

def measurements_requested(project):
    count = MeasurementResult_pichina.objects.filter(step_result__procedure_result__work_order__project=project).count()
    return count
