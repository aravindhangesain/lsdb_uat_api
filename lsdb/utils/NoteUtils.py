...
from lsdb.models import NoteReadStatus,Note_pichina
...
def get_note_counts_pichina(user, obj):
    # from lsdb.models import NoteType
    note_counts = []
    queryset = obj.notes.all()
    note_list = queryset.values_list('note_type__name').distinct()
    children = Note_pichina.objects.filter(parent_note__in=queryset).distinct()
    type_list=[]
    for note in note_list:
        if note[0] not in type_list:
            type_list.append(note[0])
