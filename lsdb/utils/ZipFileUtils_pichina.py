...
from lsdb.models import ModuleIntakeImages_pichina 
from lsdb.models import ModuleIntakeDetails_pichina
...
def create_download_file(work_orders: QuerySet[Workorder_pichina], tsd_ids, unit_ids,
    ...
    with zipfile.ZipFile(mem_zip, mode='w', compression=zipfile.ZIP_DEFLATED) as zf:
        for work_order in work_orders:
            ...
            if ModuleIntakeDetails_pichina.objects.filter(bom=work_order.name,projects=work_order.project_id).exists():
                module_intake = ModuleIntakeDetails_pichina.objects.get(bom=work_order.name,projects=work_order.project_id)
                if module_intake:
                    module_intake_images = ModuleIntakeImages_pichina.objects.filter(moduleintake_id=module_intake.id)
                    ...
                    crate_intake_images = CrateIntakeImages_pichina.objects.filter(newcrateintake_id=module_intake.newcrateintake_id)
                    ...
                    scanned_panels_data = ScannedPannels_pichina.objects.filter(module_intake_id=module_intake.id)
                    ...
                    unit_type = UnitType_pichina.objects.filter(model=module_type).first()

                    if unit_type:
                        module_property_id = unit_type.module_property_id

                        module_property = ModuleProperty_pichina.objects.filter(id=module_property_id).first()
            ...
            if tsd_ids:
                units = Unit_pichina.objects.filter(Q(procedureresult_pichina__work_order=work_order) & Q(
                    procedureresult_pichina__test_sequence_definition__id__in=tsd_ids)).distinct()
            elif unit_ids:
                units = Unit_pichina.objects.filter(
                    Q(procedureresult_pichina__work_order=work_order) & Q(id__in=unit_ids)).distinct()
            else:
                units = Unit_pichina.objects.filter(procedureresult_pichina__work_order=work_order).distinct()
            ...
            for unit in units:
                ...
                if procedure_ids:
                    procedure_query = ProcedureResult_pichina.objects.filter(
                        ...
                        'unit').prefetch_related('stepresult_pichina_set__measurementresult_pichina_set').distinct()
                else:
                    procedure_query = ProcedureResult_pichina.objects.filter(unit=unit).order_by(
                        'test_sequence_definition', 'linear_execution_group').select_related(
                        'unit').prefetch_related('stepresult_pichina_set__measurementresult_pichina_set')
                ...
                for test in procedure_query:
                    final_result = ProcedureResult_FinalResult_pichina.objects.filter(procedure_result_id=test.id).values_list('final_result', flat=True).first()
