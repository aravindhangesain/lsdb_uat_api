...
# from lsdb.serializers.Label_pichinaSerializer import Label_pichinaSerializer
from lsdb.models  import AzureFile_pichina,Note_pichina
...
class Note_pichinaSerializer(serializers.ModelSerializer):
    ...
    def create(self, validated_data):
        ...
        obj = Note_pichina.objects.create(**validated_data)
        obj.save()
        return obj

    def get_last_update_datetime(self, obj):
        # need highest date of all chlid notes:
        date_time = Note_pichina.objects.filter(parent_note=obj).aggregate(Max('datetime')).get('datetime__max')
        if date_time:
            return date_time
        else:
            return obj.datetime
    ...
    #     return bool(obj.notereadstatus_pichina_set.filter(user=user).count())
